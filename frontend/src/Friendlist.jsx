...
function Friendlist() {
    ...
    const fetchFriendData = async () => {
        try {
            const requestsInResponse = await axios.get('http://127.0.0.1:8000/friends/listRequests/', {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('authToken')}`,
                },
            });
            const requestsOutResponse = await axios.get('http://127.0.0.1:8000/friends/listOutGoingRequests/', {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('authToken')}`,
                },
            });
            const friendsListResponse = await axios.get('http://127.0.0.1:8000/friends/list/', {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('authToken')}`,
                },
            });
    ...
    const handleAcceptRequest = async (userId) => {
        try {
            await axios.post(`http://127.0.0.1:8000/friends/accept/${userId}`, {}, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('authToken')}`,
                },
            });
                    ...
                    <div>
                        ...
                        <ul>
                            {requestsIn.map((user) => (
                                <li key={user.id}>
                                    {user.name} 
                                    <button onClick={() => handleDeleteRequestSend(user.id)}>decline</button>
                                </li>
                            ))}
                        </ul>
                    </div>
                        ...
                        <ul>
                            {requestsOut.map((user) => (
                                <li key={user.id}>
                                    ...
                                    <button onClick={() => handleDeleteRequestReceive(user.id)}>Delete</button>
                                </li>
                            ))}
                        </ul>
                    </div>
    ...
    return (
        <div className="friendplace">
            ...
            {isDropdownOpen && (
                <div className="frienddropdown" ref={dropdownRef} style={{  position: 'absolute', right: 0 }}>
                    <ul>
                        <li onClick={() => { setActiveTab('requestsIn'); setIsDropdownOpen(true); }}>Requests In</li>
                        <li onClick={() => { setActiveTab('requestsOut'); setIsDropdownOpen(true); }}>Requests Out</li>
                        <li onClick={() => { setActiveTab('friendsList'); setIsDropdownOpen(true); }}>Friends List</li>
                    </ul>
                    {renderContent()}
                </div>
            )}
