...
function Header({ onlogout }) {
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                const response = await axios.get('https://your-backend-endpoint/user-data', {
                    headers: {
    ...
    const handlePhotoUpload = async () => {
        ...
        try {
            const response = await axios.put('https://your-backend-endpoint/update-photo', formData, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('authToken')}`,
    ...
    const handleSubmitUsernameChange = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put('https://your-backend-endpoint/update-username', {
    ...
    const handleSubmitInfoChange = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put('https://your-backend-endpoint/update-info', {
                info: newInfo,
            }, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('authToken')}`,
                ...
                {isDropdownOpen && (
                    <div className="dropdown" ref={dropdownRef}>
                        ...
                        <div className="dropdown-buttons">
                            <button onClick={handleEditPhoto} className="dropdown-item">Edit Photo</button>
                            <button onClick={handleEditUsername} className="dropdown-item">Edit Username</button>
                            <button onClick={handleEditInfo} className="dropdown-item">Edit Info</button>
                        </div>

                        {isEditingPhoto && (
                            <div className="edit-photo-form">
                                <input type="file" accept="image/*" onChange={handleFileChange} />
                                <button onClick={handlePhotoUpload} className="submit-button">Upload</button>
                                <button type="button" className="cancel-button" onClick={() => setIsEditingPhoto(false)}>Cancel</button>
                            </div>
                        )}

                        {isEditingUsername && (
                            <form onSubmit={handleSubmitUsernameChange} className="edit-username-form">
                                <input
                                    type="text"
                                    value={newUsername}
                        ...
                        {isEditingInfo && (
                            <form onSubmit={handleSubmitInfoChange} className="edit-info-form">
                                <input
                                    type="text"
                                    value={newInfo}
                                ...
                                />
                                <button type="submit" className="submit-button">Submit</button>
                                <button type="button" className="cancel-button" onClick={() => setIsEditingInfo(false)}>Cancel</button>
                            </form>
                        )}
                    </div>
                )}
